...
/*
 -Get Vector between two corners of eye
 -Get magnitude of that vector
 -Find slope of line between two corners, get perp slope
 -Find line with perp slope that intersects pupil point
 -Find the point that it intersects the distance line
 -Get vertical distance by subtracting POI and pupil
...
class ProcessingVC: UIViewController {
    ...
    var pupilPosLabel = UILabel()
    
    var priorPupilPos = [[CGPoint]]()
    
    let numDataPointsToSave = 50;
...
extension ProcessingVC {
    ...
    func handleFaceLandmarks(request: VNRequest, error: Error?) {
        DispatchQueue.main.async {
            ...
            self.previewView.removeMask()
            if(!results.isEmpty){
                self.newFrame(face: results[0]);//Assuming one face
            }
    ...
    func newFrame(face: VNFaceObservation){
        let leftPupilPos = averagePoints(points:(face.landmarks?.leftPupil?.normalizedPoints)!)
        let rightPupilPos = averagePoints(points:(face.landmarks?.rightPupil?.normalizedPoints)!)
        updatePupilPos(leftPos: leftPupilPos, rightPos: rightPupilPos)
        let leftEye = Eye(eye: (face.landmarks?.leftEye?.normalizedPoints)!, _pupil: (face.landmarks?.leftPupil?.normalizedPoints)!)
    ...
    func updatePupilPos(leftPos: CGPoint, rightPos: CGPoint){
        
        //Update left side
        _ = priorPupilPos[0].popLast()//Throw away last entry
        priorPupilPos[0].insert(leftPos, at: 0);//Insert new one in the beginning
        //Update right
        _ = priorPupilPos[1].popLast()
        priorPupilPos[1].insert(rightPos, at: 0);
        
        
        
    }
    func initPriorPupilPos(){
        for _ in 1...2{
            priorPupilPos.append([CGPoint](repeating: CGPoint(x: 0, y: 0), count: numDataPointsToSave))//Add an array with 50 zeros in it
        }
